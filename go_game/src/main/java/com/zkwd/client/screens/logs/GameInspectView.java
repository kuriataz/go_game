...
public class GameInspectView extends BorderPane {
  ...
  public GameInspectView(int gameID) {
    ...
    sp = new StackPane(new Label("Loading..."));
  ...
  /**
   * Converts alphabet characters to integers. (a-z -> 0-26)
   * @param c character from the Latin lowercase alphabet
   * @return integer value corresponding to it's place in the alphabet
   */
  ...
  /**
   * Load the board and game info correctly.
   */
  private void initialiseDisplay() {
    // display board
    sp.getChildren().set(0, boards.get(boards.size() - 1));
    ...
    // set display index to last board
    ind = boards.size() - 1;
    //load timestamp
